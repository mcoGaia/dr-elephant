...
import views.html.results.flowDefinitionIdDetails;
...
public class Application extends Controller {
	...
	public static final String FINISHED_TIME_BEGIN = "finished-time-begin-date";
	public static final String FINISHED_TIME_END = "finished-time-end-date";
	...
	/**
	 * Serves the initial index.html page for the new user interface. This page
	 * contains the whole web app
	 */
	...
	public static Result preflight(String all) {
		...
		response().setHeader("Access-Control-Request-Headers",
				"X-Requested-With, accept, content-type, Origin, X-Json");
		return ok();
	}
	...
	public static Result filter() {
		...
		if (exception != null && (cu4 != null || cu6 != null || cu8 != null)) // exception avec soit cu4-6-8 coche
			...
			result = query.setFirstRow((paginationBarStartIndex - 1) * pageLength)
					.setMaxRows((paginationStats.getPageBarLength() - 1) * pageLength + 1).where().conjunction()
					.eq(AppResult.TABLE.APP_HEURISTIC_RESULTS + "." + AppHeuristicResult.TABLE.HEURISTIC_NAME,
							exception)
					.disjunction().ilike(AppResult.TABLE.NAME, cu4 + "%").ilike(AppResult.TABLE.NAME, cu6 + "%")
					.ilike(AppResult.TABLE.NAME, cu8 + "%").endJunction().endJunction().order()
					.desc(AppResult.TABLE.FINISH_TIME).findList();
		else if (exception == null) // pas d'exception
			result = query.setFirstRow((paginationBarStartIndex - 1) * pageLength)
					.setMaxRows((paginationStats.getPageBarLength() - 1) * pageLength + 1).where().disjunction()
					.ilike(AppResult.TABLE.NAME, cu4 + "%").ilike(AppResult.TABLE.NAME, cu6 + "%")
					.ilike(AppResult.TABLE.NAME, cu8 + "%").endJunction().order().desc(AppResult.TABLE.FINISH_TIME)
					.findList();
		else // ttes le exceptions
			result = query.setFirstRow((paginationBarStartIndex - 1) * pageLength)
					.setMaxRows((paginationStats.getPageBarLength() - 1) * pageLength + 1).where()
					.eq(AppResult.TABLE.APP_HEURISTIC_RESULTS + "." + AppHeuristicResult.TABLE.HEURISTIC_NAME,
							exception)
					.order().desc(AppResult.TABLE.FINISH_TIME).findList();
		...
		if (result.isEmpty() || currentPage > paginationStats.computePaginationBarEndIndex(result.size())) {
			...
			return ok(filterPage.render(paginationStats, searchResults.render(
					String.format("Results: Showing %,d of %,d " + str, resultsToDisplay.size(), query.findRowCount()),
					resultsToDisplay)));
		}
	}
	...
	public static Result dashboard() {
		...
		if (now - _lastFetch > FETCH_DELAY) {
			...
			_numJobsCritical = AppResult.find.where().gt(AppResult.TABLE.FINISH_TIME, finishDate)
					.eq(AppResult.TABLE.SEVERITY, Severity.CRITICAL.getValue()).findRowCount();
			_numJobsSevere = AppResult.find.where().gt(AppResult.TABLE.FINISH_TIME, finishDate)
					.eq(AppResult.TABLE.SEVERITY, Severity.SEVERE.getValue()).findRowCount();
			_lastFetch = now;
		}

		// Fetch only required fields for jobs analysed in the last 24 hours up to a max
		// of 50 jobs
		List<AppResult> results = AppResult.find.select(AppResult.getSearchFields()).where()
				.gt(AppResult.TABLE.FINISH_TIME, finishDate).order().desc(AppResult.TABLE.FINISH_TIME).setMaxRows(50)
	...
	/**
	 * Returns the scheduler info id/url pair for the most recent app result that
	 * has an id like value (which can use % and _ SQL wild cards) for the specified
	 * field. Note that this is a pair rather than merely an ID/URL because for some
	 * schedulers (e.g. Airflow) they are not equivalent and usually the UI wants to
	 * display the ID with a link to the URL. While it is true that the URL can
	 * probably be derived from the ID in most cases, we would need scheduler
	 * specific logic which would be a mess.
	 */
	private static IdUrlPair bestSchedulerInfoMatchLikeValue(String value, String schedulerIdField) {
		...
		AppResult result = AppResult.find.select(String.format("%s, %s", schedulerIdField, schedulerUrlField)).where()
				.like(schedulerIdField, value).order().desc(AppResult.TABLE.FINISH_TIME).setMaxRows(1).findUnique();
		if (result != null) {
			if (schedulerIdField.equals(AppResult.TABLE.FLOW_DEF_ID)) {
	...
	/**
	 * Given a (possibly) partial scheduler info id, try to find the closest
	 * existing id.
	 */
	private static IdUrlPair bestSchedulerInfoMatchGivenPartialId(String partialSchedulerInfoId,
		...
		if (schedulerInfoPair == null
				&& ElephantContext.instance().getGeneralConf().getBoolean(SEARCH_MATCHES_PARTIAL_CONF, true)) {
			schedulerInfoPair = bestSchedulerInfoMatchLikeValue(String.format("%s%%", partialSchedulerInfoId),
					schedulerInfoIdField);
		}
	...
	public static Result search() {
		...
		if (!appId.isEmpty()) {
			AppResult result = AppResult.find.select("*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*")
					...
					.where().idEq(appId).findUnique();
			return ok(searchPage.render(null, jobDetails.render(result)));
		} else if (Utils.isSet(partialFlowExecId)) {
			IdUrlPair flowExecPair = bestSchedulerInfoMatchGivenPartialId(partialFlowExecId,
					AppResult.TABLE.FLOW_EXEC_ID);
			List<AppResult> results = AppResult.find
					.select(AppResult.getSearchFields() + "," + AppResult.TABLE.JOB_EXEC_ID)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).where()
					.eq(AppResult.TABLE.FLOW_EXEC_ID, flowExecPair.getId()).findList();
			Map<IdUrlPair, List<AppResult>> map = ControllerUtil.groupJobs(results,
					ControllerUtil.GroupBy.JOB_EXECUTION_ID);
			...
			List<AppResult> results = AppResult.find
					...
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).where()
					.eq(AppResult.TABLE.JOB_DEF_ID, jobDefId).findList();
			Map<IdUrlPair, List<AppResult>> map = ControllerUtil.groupJobs(results,
					ControllerUtil.GroupBy.FLOW_EXECUTION_ID);

			String flowDefId = (results.isEmpty()) ? "" : results.get(0).flowDefId; // all results should have the same
																					// flow id
	...
	public static Result delete() {
		...
		// Search and display job details when job id or flow execution url is provided.
		if (!appId.isEmpty()) {
			results = AppResult.find.select("*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(
					AppResult.TABLE.APP_HEURISTIC_RESULTS + "." + AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS,
					"*").where().idEq(appId).findIds();
		} else if (Utils.isSet(partialFlowExecId)) {
			IdUrlPair flowExecPair = bestSchedulerInfoMatchGivenPartialId(partialFlowExecId,
					AppResult.TABLE.FLOW_EXEC_ID);
			results = AppResult.find.select(AppResult.getSearchFields() + "," + AppResult.TABLE.JOB_EXEC_ID)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).where()
					.eq(AppResult.TABLE.FLOW_EXEC_ID, flowExecPair.getId()).findIds();

		} else if (!jobDefId.isEmpty()) {
			results = AppResult.find.select(AppResult.getSearchFields() + "," + AppResult.TABLE.JOB_DEF_ID)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).where()
					.eq(AppResult.TABLE.JOB_DEF_ID, jobDefId).findIds();

		} else {
			// Filter jobs by search parameters
			Query<AppResult> query = generateSearchQuery(AppResult.getSearchFields(), getSearchParams());
			results = query.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields())
					.findIds();
		...
		if (results != null) {
			...
			Ebean.execute(Ebean.createSqlUpdate(
					"delete from yarn_app_heuristic_result_details where yarn_app_heuristic_result_id in ("
							+ "select id from yarn_app_heuristic_result where yarn_app_result_id in (" + idString + ")"
							+ ")"));
			Ebean.execute(Ebean.createSqlUpdate(
					"delete from yarn_app_heuristic_result where yarn_app_result_id in (" + idString + ")"));
			Ebean.execute(Ebean.createSqlUpdate("delete from yarn_app_result where id in (" + idString + ")"));
		}

		return resetSearch();
	}

	private static Status resetSearch() {
		// Prepare pagination of results
		PaginationStats paginationStats = new PaginationStats(PAGE_LENGTH, PAGE_BAR_LENGTH);
		int pageLength = paginationStats.getPageLength();
		paginationStats.setCurrentPage(1);
		...
		// Filter jobs by search parameters
		Query<AppResult> query = generateSearchQuery(AppResult.getSearchFields(), new HashMap<String, String>());
	...
	public static Map<String, String> getSearchParams() {
		...
		try {
			if (form.get(FINISHED_TIME_BEGIN) != null && !form.get(FINISHED_TIME_BEGIN).isEmpty()) {
				searchParams.put(FINISHED_TIME_BEGIN,
						String.valueOf(df.parse(form.get(FINISHED_TIME_BEGIN)).getTime()));
			}
		} catch (ParseException e) {
			logger.error(e.getMessage());
		}
		try {
			if (form.get(FINISHED_TIME_END) != null && !form.get(FINISHED_TIME_END).isEmpty()) {
				searchParams.put(FINISHED_TIME_END, String.valueOf(df.parse(form.get(FINISHED_TIME_END)).getTime()));
			}
		} catch (ParseException e) {
			logger.error(e.getMessage());
		}
	 ...
	 * @param searchParams
	 *            The fields to query on the table
	...
	public static Query<AppResult> generateSearchQuery(String selectParams, Map<String, String> searchParams) {
		...
		// // Time Predicates. Both the startedTimeBegin and startedTimeEnd are
		// inclusive
		// // in the filter
	...
	public static Result compare() {
		...
		if (partialFlowExecId1 != null && !partialFlowExecId1.isEmpty() && partialFlowExecId2 != null
				&& !partialFlowExecId2.isEmpty()) {
			IdUrlPair flowExecIdPair1 = bestSchedulerInfoMatchGivenPartialId(partialFlowExecId1,
					AppResult.TABLE.FLOW_EXEC_ID);
			IdUrlPair flowExecIdPair2 = bestSchedulerInfoMatchGivenPartialId(partialFlowExecId2,
					AppResult.TABLE.FLOW_EXEC_ID);
			results1 = AppResult.find
					.select(AppResult.getSearchFields() + "," + AppResult.TABLE.JOB_DEF_ID + ","
							+ AppResult.TABLE.JOB_DEF_URL + "," + AppResult.TABLE.FLOW_EXEC_ID + ","
							+ AppResult.TABLE.FLOW_EXEC_URL)
	...
	/**
	 * Helper Method for the compare controller. This Compares 2 flow executions at
	 * job level.
	 *
	 * @param results1
	 *            The list of jobs under flow execution 1
	 * @param results2
	 *            The list of jobs under flow execution 2
	 * @return A map of Job Urls to the list of jobs corresponding to the 2 flow
	 *         execution urls
	 */
	private static Map<IdUrlPair, Map<IdUrlPair, List<AppResult>>> compareFlows(List<AppResult> results1,
		...
		if (results1 != null && !results1.isEmpty() && results2 != null && !results2.isEmpty()) {
			...
			Map<IdUrlPair, List<AppResult>> map1 = ControllerUtil.groupJobs(results1,
					ControllerUtil.GroupBy.JOB_DEFINITION_ID);
			Map<IdUrlPair, List<AppResult>> map2 = ControllerUtil.groupJobs(results2,
					ControllerUtil.GroupBy.JOB_DEFINITION_ID);
			...
			// Display jobs that are common to the two flows first followed by jobs in flow
			// 1 and flow 2.
	...
	private static Result getFlowHistory(Version version) {
		...
		if (!Utils.isSet(partialFlowDefId)) {
			if (version.equals(Version.NEW)) {
				return ok(flowHistoryPage.render(partialFlowDefId, graphType,
						flowHistoryResults.render(null, null, null, null)));
			} else {
		...
		if (graphType.equals("time") || graphType.equals("resources")) {
			...
			// if graph type is time or resources, we don't need the result from
			// APP_HEURISTIC_RESULTS
			results = AppResult.find
					.select(AppResult.getSearchFields() + "," + AppResult.TABLE.FLOW_EXEC_ID + ","
							+ AppResult.TABLE.FLOW_EXEC_URL + "," + AppResult.TABLE.JOB_DEF_ID + ","
							+ AppResult.TABLE.JOB_DEF_URL + "," + AppResult.TABLE.JOB_NAME)
					.where().eq(AppResult.TABLE.FLOW_DEF_ID, flowDefPair.getId()).order()
					.desc(AppResult.TABLE.FINISH_TIME).setMaxRows(JOB_HISTORY_LIMIT).findList();
		} else {
			...
			results = AppResult.find
					.select(AppResult.getSearchFields() + "," + AppResult.TABLE.FLOW_EXEC_ID + ","
							...
							+ AppResult.TABLE.JOB_DEF_URL + "," + AppResult.TABLE.JOB_NAME)
					.where().eq(AppResult.TABLE.FLOW_DEF_ID, flowDefPair.getId()).order()
					.desc(AppResult.TABLE.FINISH_TIME).setMaxRows(JOB_HISTORY_LIMIT)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).findList();
		}
		if (results.size() == 0) {
		...
		Map<IdUrlPair, List<AppResult>> flowExecIdToJobsMap = ControllerUtil.limitHistoryResults(
				ControllerUtil.groupJobs(results, ControllerUtil.GroupBy.FLOW_EXECUTION_ID), results.size(),
				MAX_HISTORY_LIMIT);
		...
		for (Map.Entry<IdUrlPair, List<AppResult>> entry : flowExecIdToJobsMap.entrySet()) {
			...
			// Reverse the list content from desc order of finish time to increasing order
			// so that when grouping we get
			...
			executionMap.put(entry.getKey(),
					ControllerUtil.groupJobs(mrJobsList, ControllerUtil.GroupBy.JOB_DEFINITION_ID));
		}

		// Calculate unique list of jobs (job def url) to maintain order across
		// executions. List will contain job def urls
		// from latest execution first followed by any other extra job def url that may
		// appear in previous executions.
		final Map<IdUrlPair, String> idPairToJobNameMap = new ListOrderedMap();

		Map<IdUrlPair, List<AppResult>> filteredTempMap = ControllerUtil.groupJobs(filteredResults,
				ControllerUtil.GroupBy.JOB_DEFINITION_ID);

		List<Map.Entry<IdUrlPair, List<AppResult>>> filteredMapList = new LinkedList<Map.Entry<IdUrlPair, List<AppResult>>>(
				filteredTempMap.entrySet());
		...
		Collections.sort(filteredMapList, new Comparator<Map.Entry<IdUrlPair, List<AppResult>>>() {
			...
			public int compare(Map.Entry<IdUrlPair, List<AppResult>> idUrlPairListMap,
				...
				return (new Long(idUrlPairListMap.getValue().get(0).finishTime))
						.compareTo(t1.getValue().get(0).finishTime);
			}
		});
		...
		if (version.equals(Version.NEW)) {
			...
			if (graphType.equals("heuristics")) {
				...
				if (hasSparkJob) {
					return notFound("Cannot plot graph for " + graphType + " since it contains a spark job. "
							+ graphType + " graphs are not supported for spark right now");
				} else {
	...
	/**
	 * Controls Job History. Displays at max MAX_HISTORY_LIMIT executions. Old
	 * version of the job history
	 */
	...
	/**
	 * Controls Job History. Displays at max MAX_HISTORY_LIMIT executions. New
	 * version of the job history
	 */
	 ...
	 *
	 * @param version
	 *            The version of job history to return
	...
	private static Result getJobHistory(Version version) {
		...
		SqlQuery q = Ebean
				.createSqlQuery("select job_def_id, count(job_def_id) as nb from yarn_app_result group by job_def_id;");
		...
		if (!Utils.isSet(partialJobDefId)) {
			if (version.equals(Version.NEW)) {
				return ok(jobHistoryPage.render(partialJobDefId, graphType,
						jobHistoryResults.render(null, null, -1, null)));
			} else {
				return ok(oldJobHistoryPage.render(partialJobDefId, graphType,
						oldJobHistoryResults.render(null, null, -1, null), jobDefList));
			}
		}
		...
		if (graphType.equals("time") || graphType.equals("resources")) {
			// we don't need APP_HEURISTIC_RESULT_DETAILS data to plot for time and
			// resources
			results = AppResult.find
					.select(AppResult.getSearchFields() + "," + AppResult.TABLE.FLOW_EXEC_ID + ","
							+ AppResult.TABLE.FLOW_EXEC_URL)
					.where().eq(AppResult.TABLE.JOB_DEF_ID, jobDefPair.getId()).order()
					.desc(AppResult.TABLE.FINISH_TIME).setMaxRows(JOB_HISTORY_LIMIT).findList();
		} else {
			...
			results = AppResult.find
					...
					.where().eq(AppResult.TABLE.JOB_DEF_ID, jobDefPair.getId()).order()
					.desc(AppResult.TABLE.FINISH_TIME).setMaxRows(JOB_HISTORY_LIMIT)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
							+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
					.findList();
		}
		...
		if (version.equals(Version.NEW)) {
			if (graphType.equals("heuristics")) {
				...
				return ok(jobHistoryPage.render(jobDefPair.getId(), graphType, jobMetricsHistoryResults
						.render(jobDefPair, graphType, executionMap, maxStages, flowExecTimeList)));
			}
		} else {
			if (graphType.equals("heuristics")) {
				...
				if (hasSparkJob) {
					...
					return ok(oldJobHistoryPage.render(jobDefPair.getId(), graphType, oldJobMetricsHistoryResults
							.render(jobDefPair, graphType, executionMap, maxStages, flowExecTimeList), jobDefList));
				}
			}
	 ...
	 *
	 * @param version
	 *            The version for which help page has to be returned
	...
	/**
	 * Rest API for searching a particular job information E.g,
	 * localhost:8080/rest/job?id=xyz
	 */
	public static Result restAppResult(String id) {
		...
		AppResult result = AppResult.find.select("*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(
				AppResult.TABLE.APP_HEURISTIC_RESULTS + "." + AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS,
	...
	public static Result getCurrentRunParameters() {
		...
		try {
			...
			String optimizationAlgo = paramValueMap.get("optimizationAlgo") == null
			...
			if (paramValueMap.containsKey("allowedMaxResourceUsagePercent")) {
				allowedMaxResourceUsagePercent = Double
						.parseDouble(paramValueMap.get("allowedMaxResourceUsagePercent"));
			}
			if (paramValueMap.containsKey("allowedMaxExecutionTimePercent")) {
				allowedMaxExecutionTimePercent = Double
						.parseDouble(paramValueMap.get("allowedMaxExecutionTimePercent"));
			}
	...
	/**
	 * Rest API for searching all jobs triggered by a particular Scheduler Job E.g.,
	 * localhost:8080/rest/jobexec?id=xyz
	 */
	public static Result restJobExecResult(String jobExecId) {
		...
		List<AppResult> result = AppResult.find.select("*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*")
				.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
						+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
				.where().eq(AppResult.TABLE.JOB_EXEC_ID, jobExecId).findList();
	...
	/**
	 * Rest API for searching all jobs under a particular flow execution E.g.,
	 * localhost:8080/rest/flowexec?id=xyz
	 */
	public static Result restFlowExecResult(String flowExecId) {
		...
		List<AppResult> results = AppResult.find.select("*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*")
				.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
						+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
				.where().eq(AppResult.TABLE.FLOW_EXEC_ID, flowExecId).findList();

		if (results.size() == 0) {
			return notFound("Unable to find record on flow exec url: " + flowExecId);
		}

		Map<IdUrlPair, List<AppResult>> groupMap = ControllerUtil.groupJobs(results,
				ControllerUtil.GroupBy.JOB_EXECUTION_ID);
	...
	public static Result restSearch() {
		...
		List<AppResult> results = query.setFirstRow((page - 1) * REST_PAGE_LENGTH).setMaxRows(REST_PAGE_LENGTH)
				.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
						+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
				.findList();
	...
	/**
	 * The Rest API for Compare Feature E.g.,
	 * localhost:8080/rest/compare?flow-exec-id1=abc&flow-exec-id2=xyz
	 */
	public static Result restCompare() {
		...
		if (flowExecId1 != null && !flowExecId1.isEmpty() && flowExecId2 != null && !flowExecId2.isEmpty()) {
			results1 = AppResult.find.select("*").where().eq(AppResult.TABLE.FLOW_EXEC_ID, flowExecId1).setMaxRows(100)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
							+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
					.findList();
			results2 = AppResult.find.select("*").where().eq(AppResult.TABLE.FLOW_EXEC_ID, flowExecId2).setMaxRows(100)
					.fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS + "."
							+ AppHeuristicResult.TABLE.APP_HEURISTIC_RESULT_DETAILS, "*")
					.findList();
		}
	...
	public static Result restFlowGraphData(String flowDefId) {
		...
		// Compute the graph data starting from the earliest available execution to
		// latest
		...
		for (int i = keyList.size() - 1; i >= 0; i--) {
			...
			Map<IdUrlPair, List<AppResult>> jobDefIdToJobsMap = ControllerUtil.groupJobs(mrJobsList,
					ControllerUtil.GroupBy.JOB_DEFINITION_ID);

			// Compute the execution records. Note that each entry in the jobDefIdToJobsMap
			// will have at least one AppResult
	...
	/**
	 * The data for plotting the job history graph using time and resource metrics.
	 * While plotting the job history graph an ajax call is made to this to fetch
	 * the graph data.
	 *
	 * Data Returned:
	 * 
	 ...
	 *         eg.
	 *         [{"user":"bmr","resourceUsed":168030208,"resourceWasted":27262750},
	...
	/**
	 * Rest data to plot flot history graph using time and resource metrics. While
	 * plotting the flow history graph an ajax call is made to this to fetch the
	 * graph data. [ { "flowtime": 1461744881991, "runtime": 3190223, "waittime":
	 * 368011, "resourceused": 180488192, "resourcewasted": 0, "jobmetrics": [ {
	 * "runtime": 3190223, "waittime": 368011, "resourceused": 180488192,
	 * "resourcewasted": 0, "jobdefurl": "sampleURL" "jobexecurl": "sampleURL" } ]
	 * }, { "flowtime": 1461818409959, "runtime": 897490, "waittime": 100703,
	 * "resourceused": 12863488, "resourcewasted": 0, "jobmetrics": [ { "runtime":
	 * 897490, "waittime": 100703, "resourceused": 12863488, "resourcewasted": 0,
	 * "jobdefurl": "sampleURL" "jobexecurl": "sampleURL" } ] } ]
	 **/
	...
	private static List<AppResult> getRestJobAppResults(String jobDefId) {
		List<AppResult> results = AppResult.find
				.select(AppResult.getSearchFields() + "," + AppResult.TABLE.FLOW_EXEC_ID + ","
						+ AppResult.TABLE.FLOW_EXEC_URL)
				.where().eq(AppResult.TABLE.JOB_DEF_ID, jobDefId).order().desc(AppResult.TABLE.FINISH_TIME)
				.setMaxRows(JOB_HISTORY_LIMIT).fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, "*").findList();

		return results;
	}

	/**
	 * Returns the list of AppResults after quering the FLOW_DEF_ID from the
	 * database
	 * 
	 * @return The list of AppResults
	 */
	private static List<AppResult> getRestFlowAppResults(String flowDefId) {
		// Fetch available flow executions with latest JOB_HISTORY_LIMIT mr jobs.
		List<AppResult> results = AppResult.find.select("*").where().eq(AppResult.TABLE.FLOW_DEF_ID, flowDefId).order()
				.desc(AppResult.TABLE.FINISH_TIME).setMaxRows(JOB_HISTORY_LIMIT).findList();
	...
	/**
	 * Returns the list of users with their resourceUsed and resourceWasted Data for
	...
	private static Collection<AppResourceUsageData> getUserResourceUsage(Date start, Date end) {
		...
		List<AppResult> results = AppResult.find.select("*").where().ge(AppResult.TABLE.START_TIME, start.getTime())
