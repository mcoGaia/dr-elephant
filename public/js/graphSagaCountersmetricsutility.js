...
function plotter(graphData, jobDefList) {
    ...
    /* SAGA Counters */
    var listSagaInput = document.getElementsByName("inputSagaCounters");
    var listSagaOutput = document.getElementsByName("outputSagaCounters");
    var tabIn = [];
    var tabOut = [];
    for(var i= 0; i < listSagaInput.length; i++)
      tabIn[i] = parseInt(listSagaInput[i].innerHTML, 10);
    for(var i= 0; i < listSagaOutput.length; i++)
      tabOut[i] = parseInt(listSagaOutput[i].innerHTML, 10);
    var max = 0;                                              //max = valeur maximale du tableau des input
    for(var i= 0; i < tabIn.length; i++)
      if (tabIn[i] > max)
        max = tabIn[i];
    var max2 = 0;                                             //max2 = valeur maximale du tableau des output
    for(var i= 0; i < tabOut.length; i++)
      if (tabOut[i] > max2)
        max2 = tabOut[i];
    var yScale = Math.max(max, max2);
...
var l=tabIn.length;
    ...
    var lineFunc = d3.svg.line()
        ...
        .y(function (d) { k=(k>0?k-1:0); return yRange(tabOut[k]); })
        .interpolate('linear');
    ...
    var lineWastedFunc = d3.svg.line()
        .x(function (d) { return xRange(d.flowtime); })
        .y(function (d) { l=(l>0?l-1:0); return yRange(tabIn[l]); })
        .interpolate('linear');
    ...
    var yAxis = d3.svg.axis()
        ...
        .tickFormat(function(d) {    //faire arriver les saga counters
                return d;          
        })
        .orient("left");
    ...
    // Add the x-axis
    graphContainer.append("svg:g")  //ajout d'un groupe
        .attr("class", "x axis")
    ...
    graphContainer.append("svg:g")
        ...
        .call(yAxis)
        .selectAll("text")
        .attr("fill", "rgb(0, 119, 181)");    //couleurs des valeurs sur l'axe y

    // Add label for the y axis
    graphContainer.append("svg:text")
        .style("font-size", "16px")
        .style("fill", "#606060")    //couleur du label
...
l=tabIn.length;
    ...
    graphContainer.append("svg:g")
        ...
        .style({stroke: 'white', fill: '#0077b5'})   //bleu
        .attr("cx", function (d) { return xRange(d.flowtime); } )
        .attr("cy", function (d) { k=(k>0?k-1:0); return yRange(tabOut[k]); } )
        .attr("r", 4);
    ...
    graphContainer.append("svg:g")
        ...
        .data(graphData)
        .enter().append("svg:circle")
        .style({stroke: 'white', fill: '#FF0000'})  //rouge
        .attr("cx", function (d) { return xRange(d.flowtime); } )
        .attr("cy", function (d) { l=(l>0?l-1:0); return yRange(tabIn[l]);} )
        .attr("r", 4);
    ...
    graphContainer.append("text")
        ...
        .attr("dy", ".30em")
        .style("text-anchor", "end")
        .text(function(d) { return "Total out" });
    ...
    graphContainer.append("text")
        ...
        .attr("dy", ".30em")
        .style("text-anchor", "end")
        .text(function(d) { return "Total in" });
    ...
    function mousemove(d) {
        ...
        if (graphData.length == 1) {
            ...
            var xValueMouse = xRange.invert(MARGINS.left + d3.mouse(this)[0]),
                ...
                index = (tabIn.length-1) - index;
                
            record = xValueMouse - dleft.flowtime > dright.flowtime - xValueMouse ? dright : dleft;
            record_in = xValueMouse - dleft.flowtime > dright.flowtime - xValueMouse ? tabIn[index] : tabIn[index+1];//
            record_out = xValueMouse - dleft.flowtime > dright.flowtime - xValueMouse ? tabOut[index] : tabOut[index+1];//                        
        }

        // Add content to tooltip
        var graphTooltip = document.getElementById("graph_tooltip");
        graphTooltip.innerHTML = '';
        graphTooltip.appendChild(getGraphTooltipContent(record, record_in, record_out));

        // Set position of highlighted circle
        tooltip.select("circle")
            .style("display", "inline")
            .attr("transform", "translate(" + xRange(record.flowtime) + "," + yRange(/*record.runtime*/record_out) +")");

        tooltip.select("circle.bluecircle")
            .style("display", "inline")
            .attr("transform", "translate(" + xRange(record.flowtime) + "," + yRange(/*record.waittime*/record_in) +")");    //point rouge

        // Set position of tooltip.
        var x = xRange(record.flowtime) - (tooltipWidth) - 10;
        var y = yRange(/*record.runtime*/record_in) - tooltip.select("body").style("height").replace("px", "")/2;
        ...
        if ((yRange(/*record.runtime*/record_in) + tooltip.select("body").style("height").replace("px", "")/2) >= yRange(0)) {
            y = yRange(/*record.runtime*/record_in) - tooltip.select("body").style("height").replace("px", "") - 10;
        }
